...
public class GerarMensalidadeController extends Funcionalidades implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		this.cb_paciente
				.setConverter(new ConverterDados(ConverterDados.GET_PACIENTE_NOME).getPacienteConverter(cb_paciente));
	...
	private void onSave() {
		...
		if (flag == 1) {
			...
			LocalDate dataSelecionada = dp_data.getValue();
			mensalidadeModel.setMes(dataSelecionada.getMonth().getDisplayName(TextStyle.FULL, Locale.getDefault()));
	...
	private void onDelete() {
		if (tabela_mensalidade.getSelectionModel().getSelectedItem() != null) {
			if (DialogFX.showConfirmation("Deseja Excluir ?")) {
				this.mensalidadeModel = tabela_mensalidade.getItems()
						.get(tabela_mensalidade.getSelectionModel().getSelectedIndex());
				if (MensalidadeDAO.executeUpdates(mensalidadeModel, MensalidadeDAO.DELETE)) {
					tabela_mensalidade.getItems().remove(tabela_mensalidade.getSelectionModel().getSelectedIndex());
