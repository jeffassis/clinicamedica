...
public class AutoCompleteComboBox implements EventHandler<KeyEvent> {
	...
	public AutoCompleteComboBox(ComboBox comboBox) {
		...
		/*
		 * Altero o evento para ele ouvir esta classe uso o KeyReleased pq o
		 * evento é chamado só apos a palavra está digitada
		 */
	...
	public void handle(KeyEvent event) {
		...
		if (salvouDados) {
			...
			if (event.getCode() == KeyCode.RIGHT || event.getCode() == KeyCode.LEFT || event.isControlDown()
				...
				/*
				 * Se for utilizado qualquer uma dessas Teclas paramos o código
				 * aqui
				 */
				return;
			}
			/*
			 * Essa lista vai conter os dados que vai ser exibido conforma o
			 * usuario digitar
			 */
			ObservableList lista = FXCollections.observableArrayList();
			for (int i = 0; i < dados.size(); i++) {
				/*
				 * Como não sabemos qual o tipo do combobox, usamos o
				 * getConverter para retornar a StringConverter que foi
				 * adicionado nele por meio da Classe ConverterDados, tendo essa
				 * String podemos comparar as Inicias dos nome, com o nome
				 * digitado no comboBox utilizamos o metodo toLowerCase que
				 * transforma todas as letras em maiusculas.
				 */
				if (comboBox.getConverter().toString(dados.get(i)).toLowerCase()
						.startsWith(comboBox.getEditor().getText().trim().toLowerCase())) {
					/*
					 * os Nome que tiveram as mesmas inicias serao adicionadas a
					 * lista
					 */
			...
			/*
			 * Salva o texto digitado no comboBox pq vamos utilizar o setItens
			 * ai o texto seria perdido
			 */
			String textoDigitado = comboBox.getEditor().getText();

			/*
			 * Alteramos para os itens dessa nova lista, assim ficando somente
			 * os que foram encontrados na condição acima (if)
			 */
			...
			System.err.println("ERRO: AutoCompleteComboBox\n"
					+ "Método saveData não foi chamado!\nÉ obrigatorio a chamada do método saveData");
		}
	}
	 ...
	 * @param comboBox
	 * @throws - Caso o comboBox esteja vázio.
	 */
	public void addComboBox(ComboBox comboBox) throws NullPointerException {
